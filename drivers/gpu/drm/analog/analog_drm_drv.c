...
static struct i2c_adapter *ddc_adapter_culo;
...
static int analog_drm_load(struct drm_device *dev, unsigned long flags)
{
	...
	private->ddc_adapter = ddc_adapter_culo;
...
static int analog_drm_attach_i2c_adapter(struct device *dev, void *data)
{
	...
	if (/*ddc_adapter_culo && */slave_adapter)
		return 0;

	of_node = analog_drm_pdev->dev.of_node;


/*	if (dev->of_node == of_parse_phandle(of_node, "ddc_adapter_culo", 0))
		ddc_adapter_culo = to_i2c_adapter(dev);
...
static struct platform_driver adv7511_encoder_driver = {
	...
	.remove = (analog_drm_platform_remove),
};
